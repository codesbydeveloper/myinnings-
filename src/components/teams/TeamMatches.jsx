...
export default function TeamMatches({ team, matches }) {
  ...
  return (
    <div className="space-y-6">
      <MatchGroup title="Upcoming" matches={upcoming} teamName={team.name} />
      <MatchGroup title="Completed" matches={completed} teamName={team.name} showResult />
    </div>
  )
}

function MatchGroup({ title, matches, teamName, showResult = false }) {
  return (
    <section>
      ...
      {matches.length ? (
        <div className="space-y-3">
          {matches.map((match) => {
            const opponent = opponentOf(match, teamName)
            const resultText =
        ...
        <EmptyDashboardState
          icon="matches"
          title={title === 'Upcoming' ? 'No upcoming matches' : 'No completed matches'}
          description={
            title === 'Upcoming'
              ? 'This team does not have any upcoming fixtures yet.'
